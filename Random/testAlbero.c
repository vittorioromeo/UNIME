...
Albero* albero_getFiglioSx(Albero* mA) { assert(mA != NULL); return mA->sx; }
Albero* albero_getFiglioDx(Albero* mA) { assert(mA != NULL); return mA->dx; }
...
{
	return albero_getSuccPred(mA, &albero_getMinimo,
		&albero_getFiglioDx, &albero_getFiglioSx);
}

Albero* albero_getPredecessore(Albero* mA)
{
	return albero_getSuccPred(mA, &albero_getMassimo,
		&albero_getFiglioSx, &albero_getFiglioDx);
}
