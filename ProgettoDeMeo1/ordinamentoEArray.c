...
int getMinArrayElementIdx(int* mArray, int mLB, int mUB)
{
    ...
    int iMin;
    for(iMin = mLB; mLB < mUB; ++mLB)
        if(mArray[mLB] < mArray[iMin]) iMin = mLB;
    return iMin;
}
...
void selectionSort(int* mArray, int mSize)
{
    ...
    int i;
    for(i = 0; i < mSize; ++i)
        swapInt(&mArray[i], &mArray[getMinArrayElementIdx(mArray, i, mSize)]);
}
...
|
|	getMinArrayElementIdx() has (N) worst time complexity, if the lower bound
is 0 and the upper bound is
|	the array size.
|
|	/---- selectionSort()
|	|
...
|
|	selectionSort() calls getMinArrayElementIdx() (N) times. Therefore, the
worst case complexity for the
...
/// @brief Returns whether a pair of number whose sum equals `mDesiredValue`
/// exists in `mArray`.
/// @details Brute force version. `mArray` can be unsorted. O(N^2) computational
/// complexity.
...
/// @return Returns -1 if the element wasn't found, the index of the element
/// otherwise.
...
{
    // Strategy: pick a pivot, then use binary search to look for `mDesiredValue
    // - pivot` in the array
...
/// @brief Prints the smallest sequence greater than `mDesiredValue` found in
/// `mArray`.
