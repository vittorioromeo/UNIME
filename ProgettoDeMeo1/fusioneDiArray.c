...
int arrayUnionInc(
    int* mAInc, int mAIncSize, int* mADec, int mADecSize, int* mATarget)
{
    ...
    while(itrInc < mAIncSize && itrDec >= 0)
    {
        ...
        if(a == b)
        {
            ...
            {
            } // Skip duplicates in source arrays
...
|	\---------------------------/
|
|	The computational complexity is obviously linear, O(max(mAIncSize,
mADecSize)).
|   In the case that mAIncSize >= mADecSize, then the worst complexity is
O(mAIncSize);
|   In the case that mAIncSize < mADecSize, then the worst complexity is
O(mADecSize);
|
...
int main()
{
    ...
    // Print union inc
    {
        ...
        vlc_resetFmt();
        int aTargetNewSize =
            arrayUnionInc(aInc, aIncSize, aDec, aDecSize, aTarget);
    ...
    // Print union dec
    {
        ...
        vlc_resetFmt();
        int aTargetNewSize =
            arrayUnionDec(aInc, aIncSize, aDec, aDecSize, aTarget);
        for(i = 0; i < aTargetNewSize; ++i) printf("%d, ", aTarget[i]);
        printf("\n");
    }
