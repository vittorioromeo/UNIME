...
double calculateVariableMonthlyRate(double mDebt, double mRate) { return mDebt * mRate; }
...
DebtResult getRemainingDebt(double mDebt, double mRate, double mAnnualInterest, double(*mRateFn)(double, double))
{
	DebtResult result;
...
double calculateRequiredFixedRate(double mDebt, double mAnnualInterest, int mExpectedMonths)
{
	...
	// The upper bound is calculated by assuming full interest on the starting debt value.
	double lb = mDebt / mExpectedMonths;
	double ub = (mDebt + (mDebt * (monthlyInterest * mExpectedMonths))) / mExpectedMonths;
	...
	for(; lb < ub; lb += 10.0)
	{
		// I check how many months it will take to pay off the loan with the current rate.
		m = 0;
		currentResult.remainingDebt = mDebt;
		
		do
		{
			currentResult = getRemainingDebt(currentResult.remainingDebt, lb, mAnnualInterest, &calculateFixedRate);
			++m;

			// If we're still in this loop, it means we haven't exceeded the expected months.
			// Therefore, if the debt was paid off, this is the optimal rate to use.
	...
	// If `lb` and `ub` do not differ much, the algorithm will run less iterations.
	// If `lb` and `ub` differ a lot, the algorithm will run many more iterations.
...
void printDebtForAnYear(double mDebt, double mAnnualInterest, double mVariableMonthlyRate)
{
	...
	for(m = 0; m < 12; ++m)
	{
		DebtResult currentResult = getRemainingDebt(mDebt, mVariableMonthlyRate, mAnnualInterest, &calculateVariableMonthlyRate);
...
void printDebtExpectedMonthsFixedRate(double mDebt, double mAnnualInterest, double mFixedRate, int mExpectedMonths)
{
	...
	do
	{
		currentResult = getRemainingDebt(mDebt, mFixedRate, mAnnualInterest, &calculateFixedRate);
		mDebt = currentResult.remainingDebt;
	...
	printf("Possible in %d months? %s\n", mExpectedMonths, (m > mExpectedMonths) ? "No" : "Yes");
